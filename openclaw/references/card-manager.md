...
1. `gpca_get_captcha` — get captcha image for registration
...
3. Ask for: email, username, login code, confirm login code, and optional referral code
...
## Login (Existing User)
1. Ask for email and login code
...
4. `gpca_reset_password` with email, verification code, and new login code
5. Proceed to login flow

NEVER store or display login details after use. If any tool returns "Session expired", restart auth.

## Apply for a Bank Card
1. `gpca_check_kyc` — verify identity verification is approved. If not, guide user through verification first
...
## Deposit to Wallet (Receive funds)
1. `gpca_supported_chains` — list available transfer networks
2. Ask user which network to use (or pick default)
3. `gpca_deposit_address` with chain_id — get wallet address
4. Display the address clearly. Tell user to send funds to this address on the selected network
5. Warn: only send on the correct network, wrong network = lost funds

## Transfer Wallet Funds to Bank Card
1. `gpca_wallet_balance` — show current wallet balance
2. `gpca_bank_card_list` — list eligible cards
3. Ask user: which card and how much to transfer
4. **CONFIRM** the operation: "Transfer X funds (≈ $X USD) to card ending in XXXX?"
5. Only after user confirms: `gpca_deposit_to_card` with card_id and amount
6. Show result

## Check Balances
- Wallet: `gpca_wallet_balance`
...
- Get security code: `gpca_get_cvv` — warn about security before revealing

## Identity Verification
1. `gpca_check_kyc` — check current status
2. If needed: `gpca_request_kyc` (Mastercard) or `gpca_request_kyc_visa` (Visa)
3. Upload documents: `gpca_add_kyc_file` with image file data
