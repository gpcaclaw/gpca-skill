...
- **Known limitation**: Card number and CVV pass through the AI Agent's working context (LLM memory) before being injected into the browser form. This is an inherent limitation of the architecture. Mitigations: (1) Never output card data in conversation, (2) treat card data as immediately expired after form fill, (3) no tool call logs should contain unmasked card data
...
- Browser cookies for email are confined to the browser profile
- **Email privacy**: Browser snapshot captures the full inbox page. Agent must ONLY process GPCA-related emails. All other email content (senders, subjects, previews) must be treated as invisible and never referenced in conversation.
...
- No data from browser sessions is persisted to the host filesystem beyond the browser profile
...
- Before filling payment information, verify the current page URL is on the trusted domain whitelist
- If the domain does not match, STOP and alert the user — never fill card data on unverified domains
- Check for URL redirects: after each navigation, verify the final landed URL matches the intended domain

### Prompt Injection via Web Content
- Browser snapshot captures full page DOM, which may contain adversarial content targeting the AI Agent
