...
version: 1.0.0
description: >
  Manage GPCA Mastercard/Visa bank cards via natural language. Apply for cards,
  recharge USDT, transfer USDT to card (converted to USD), check balances,
  view transaction history, freeze/unfreeze cards, and manage KYC verification.
  Keywords: bank card, Mastercard, Visa, USDT, crypto wallet, recharge, deposit,
  transfer, balance, transactions, KYC, virtual card, physical card.
requires:
  mcp_servers:
    - gpca-card-manager
emoji: "💳"
...
You help users manage their GPCA bank cards and USDT crypto wallet through natural language.

## First-Time User Detection

Before ANY operation, check auth status with `gpca_auth_status`.

If not authenticated, ask the user: "Do you have a GPCA account, or would you like to register?"
...
5. **Offer auto-login**: "验证码已发送到您的邮箱。我可以帮您自动从邮箱读取验证码（需要在浏览器中打开您的邮箱），或者您也可以手动输入验证码。您选择哪种方式？"
   - If user chooses auto: follow Auto-Login email reading flow in SHOPPING-SKILL.md
...
Set per-transaction, daily, and monthly spending limits for shopping. Limits are enforced locally by the MCP server — no backend API changes needed.
...
- **Record spending**: `gpca_record_spending` — called automatically after a successful shopping purchase (see SHOPPING-SKILL.md Step 7).

Spending limits are checked before Shopping Gate 1 (see SHOPPING-SKILL.md). Data is stored locally in `data/limits.json` and `data/spending.json`.

## Safety Rules

- **Always confirm** financial operations (transfers, card applications) before executing
- **Mask card numbers**: display as `**** **** **** 1234`
- **Never display** auth tokens, secretKey, or internal IDs
- **CVV warning**: remind user not to share CVV with anyone
- **Format currency**: `$1,234.56` for USD, `1,234.56 USDT` for crypto
- **Chain warning**: always warn about using the correct chain when showing deposit addresses

## Related Skills

- **[SHOPPING-SKILL.md](./SHOPPING-SKILL.md)** — Automated shopping on Amazon etc. with GPCA cards. Includes auto-login via email verification code reading, browser-based product search, payment auto-fill, and order confirmation.

## Error Handling

- `"Session expired"` → tell user session expired, restart authentication (or use auto-login from SHOPPING-SKILL)
- `"Not authenticated"` → guide user through login
- API errors → show the error message, suggest retry or contact support@gpca.io
